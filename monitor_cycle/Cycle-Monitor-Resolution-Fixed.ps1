# Cycle-Monitor-Resolution-Fixed.ps1
...
function Get-AvailableDisplays {
    ...
    try {
        ...
        # WMI failed, fall back to enumeration
    }
    
    # Fallback: enumerate displays manually
    if ($displays.Count -eq 0) {
        $i = 1
        while ($i -le 10) {  # Limit to 10 displays
...
if (-not $DisplayName) {
    $availableDisplays = Get-AvailableDisplays
    if ($availableDisplays.Count -eq 0) {
        Write-Error "No displays detected. Please specify a display name manually."
        exit 1
    }
    
    # Use the first available display
    $DisplayName = $availableDisplays[0].Name
    Write-Host "Auto-detected display: $DisplayName ($($availableDisplays[0].Width)x$($availableDisplays[0].Height) @ $($availableDisplays[0].Frequency)Hz)"
}

# Define the C# code as a string first, then use Add-Type
$csharpCode = @"
using System;
...
try {
    ...
    Write-Host "Current mode: $($cur.W)x$($cur.H) @ $($cur.F)Hz"
    ...
    Write-Host "Switching to: $($next.W)x$($next.H) @ $($next.F)Hz"
